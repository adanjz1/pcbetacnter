...
if($_REQUEST['mode']=='status')
{
	...
	$row_status=mysql_fetch_array(mysql_query("select * from ".MANAGE_EMAILTEMPLATE." where content_id='".$content_id."'"));
	
	if($row_status[is_active]==1)
	{
		mysql_query("update ".MANAGE_EMAILTEMPLATE." set is_active=0 where content_id='".$content_id."'");
	}
	else
	{
		mysql_query("update ".MANAGE_EMAILTEMPLATE." set is_active=1 where content_id='".$content_id."'");
	}
...
elseif($_REQUEST['mode']=='delete')
{
	$content_id = $_REQUEST['content_id'];
	$row_status=mysql_fetch_array(mysql_query("select * from ".MANAGE_EMAILTEMPLATE." where content_id='".$content_id."'"));
	mysql_query("delete from ".MANAGE_EMAILTEMPLATE." where content_id='".$content_id."'");
...
elseif($_REQUEST['mode']=='update')
{
	...
	{
		$content_id = $_REQUEST['content_id'];
		$db->query_update(MANAGE_EMAILTEMPLATE, $data, "content_id='$content_id'");
		$msg = "Successfully Edit ".$_REQUEST['content_name'];
	}
	else
	{
		$content_id=$db->query_insert(MANAGE_EMAILTEMPLATE, $data);
...
<div id="content">
	<div id="main">
		...
		<?php
			...
			$sql="select * from ".MANAGE_EMAILTEMPLATE." ORDER BY content_name";
			$sqlStrAux = "SELECT count(*) as total FROM ".MANAGE_EMAILTEMPLATE." ORDER BY content_name";
		...
		<tr class="<?=$id?>">
			...
			<td width="15%" align="center"><a href="emailtemplate.php?content_id=<?=$row['content_id']?>&mode=status&page=<?=$_REQUEST['page']?>"><?php if($row['is_active']==1){?>ON<?php } else { ?><font color="#FF0000">OFF</font><?php }?></a></td>
			<td width="11%" align="center"><a href="emailtemplate.php?content_id=<?=$row['content_id']?>&mode=edit&page=<?=$_REQUEST['page']?>">Edit</a></td>
			<td width="10%" align="center"><a href="emailtemplate.php?content_id=<?=$row['content_id']?>&mode=delete&page=<?=$_REQUEST['page']?>" onClick='return confirm("Are you sure to delete this Content?")'>Delete</a></td>
		</tr>
		<?php
					$i++;
				}
		...
		<?php if($_REQUEST['mode']=='edit'){ $mode = "Edit Content"; } elseif($_REQUEST['mode']=='add'){ $mode = "Add Content"; }
				$sql = mysql_query("SELECT * FROM ".MANAGE_EMAILTEMPLATE." WHERE content_id='".$_REQUEST['content_id']."'");
