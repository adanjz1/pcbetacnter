...
{
$from="www.pccounter.net<".$_POST['from'].">";
 $subject=$_POST['subject'];
 $message=$_POST['message'];

mysql_query("INSERT INTO `pc_counter_newslettermanager` (`newsletter` ,`subject`)VALUES ('$message', '$subject')");

	$reg_id=explode('-',$_REQUEST['reg_id']);
	for($i=1;$i<count($reg_id);$i++)
	{
	  $row=mysql_fetch_object(mysql_query("SELECT * FROM ".MANAGE_DAILY_DEAL_EMAIL." where deal_email_id='".$reg_id[$i]."'"));
	  $to=$row->deal_email;
...
mysql_query("update `pc_counter_newslettermanager` set `newsletter`='$message' where id='$id' ");
...
$rownews=mysql_fetch_array(mysql_query("select * from `pc_counter_newslettermanager` where id='".$_REQUEST['id']."'"));
?>
...
<tr>
   ...
   <tr>
	<td height="34" colspan="2" align="center" valign="middle">
									...
									<td  width="91%"><input name="from" value="info@pc counter.net" type="text" readonly></td>
									</tr>
									<tr>
									...
									<?
									 $resn=mysql_query("select * from pc_counter_newslettermanager");
