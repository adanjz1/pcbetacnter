...
if($_REQUEST['mode']=='delete')
{
	$id = $_REQUEST['id'];
	//echo $id;
	$row_status=mysql_fetch_array(mysql_query("select * from ".MANAGE_REQUESTINFORMATION." where id='".$id."'"));
	mysql_query("delete from ".MANAGE_REQUESTINFORMATION." where id='".$id."'");
	$msg = "Successfully Delete ".$row_status['email'];
	$_REQUEST['mode']="";
}
?>
<div id="bgwrap">
...
<div id="content">
	<div id="main">
		...
		<tr>
			...
			<td width="39%" align="left" id="cat_trh">User Email</td>
			<td width="18%" align="center" id="cat_trh">Website URL</td>
		...
		<?php
			...
			$sql="select * from ".MANAGE_REQUESTINFORMATION;
			$sqlStrAux = "SELECT count(*) as total FROM ".MANAGE_REQUESTINFORMATION;
		...
		<tr class="<?=$id?>">
			...
			<td width="41%" align="left"><?=$row['email']?></td>
			<td width="18%" align="center"><?=$row['websiteurl']?></td>
			<td width="18%" align="center"><?=$row['status']?></td>
			<?php /*?><td width="16%" align="center"><a href="country_manager.php?country_id=<?=$row['id']?>&mode=edit&page=<?=$_REQUEST['page']?>">Edit</a></td><?php */?>
			<td width="16%" align="center"><a href="request_info.php?id=<?=$row['id']?>&mode=delete&page=<?=$_REQUEST['page']?>" onClick='return confirm("Are you sure to delete this Article?")'>Delete</a></td>
		</tr>
		<?php
					$i++;
				}
