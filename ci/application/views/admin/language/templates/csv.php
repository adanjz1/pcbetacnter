    ...
    foreach ($aryHeader as $v1) {
        ...
        switch ($this->form[$v1]['element'][0]) {
            ...
            case 'radio':
                ...
                if (!is_array($__value)){
                	if (isset($e[1]) && !empty($__value)) {
                		$tmp[] = (isset($options[$__value])) ? $options[$__value] : $__value;
                	}
                }else{
                	$tmp1 = '';
                	$sp = '';
                	foreach ($__value as $sv){
                		$tmp1 .= $sp.((isset($options[$sv])) ? $options[$sv] : $sv);
                		$sp = ',';
                	}
                	$tmp[] = $tmp1;
                }
