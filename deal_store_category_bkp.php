				...
				<div class="latest_deal">
					...
					<div class="left_mid">
						<?php
							...
							if($_REQUEST['brand'] != '')
							{
								$sql_deal = "select * from ".TABLE_PRODUCT." where deal_coupon = 'd' and deal_sources_id = '".$_REQUEST['brand']."' and is_active = 1 order by product_id desc ".$GLOBALS[sql_page]."";
								$sqlStrAux = "SELECT count(*) as total FROM ".TABLE_PRODUCT." where deal_coupon = 'd' and deal_sources_id = '".$_REQUEST['brand']."' and is_active = 1";
							}
							elseif($_REQUEST['category'] != '')
							{
								$sql_deal = "select * from ".TABLE_PRODUCT." where deal_coupon = 'd' and cat_id = '".$_REQUEST['category']."' and is_active = 1 order by product_id desc ".$GLOBALS[sql_page]."";
								$sqlStrAux = "SELECT count(*) as total FROM ".TABLE_PRODUCT." where deal_coupon = 'd' and  cat_id = '".$_REQUEST['category']."' and is_active = 1";
							}
							else
							{
								$sql_deal = "select * from ".TABLE_PRODUCT." where deal_coupon = 'd' and is_active = 1 order by product_id desc ".$GLOBALS[sql_page]."";
								$sqlStrAux = "SELECT count(*) as total FROM ".TABLE_PRODUCT." where deal_coupon = 'd' and is_active = 1";
							}
						...
						?>
										<div class="pro_box">
											...
											<div class="pro_mid">
												...
												?>
												<h1 style="height:43px;"><a href="<?php echo SITE_URL; ?>deal_detail.php?action=review&productid=<?php echo $row_deal['product_id']; ?>" style="color: #2B2B2B; font: bold 16px/20px Arial,Helvetica,sans-serif;	margin: 0;	padding: 0; text-transform: none; text-decoration: none;"><?php echo wrptxt(stripslashes($row_deal['title']),50); ?></a></h1>
												<div class="border_bg"><a href="<?php echo SITE_URL; ?>deal_detail.php?action=review&productid=<?php echo $row_deal['product_id']; ?>"><img src="<?php echo $row_deal['image_url']; ?>" alt="" width="188" height="137" border="0"/></a></div>
												<ul>
