				...
				<div class="latest_deal">
					...
					<div class="left_mid">
						<?php
							...
							if($_REQUEST['brand'] != '')
							{
								$sql_deal = "select * from ".TABLE_PRODUCT." where deal_coupon = 'd' and deal_sources_id = '".$_REQUEST['brand']."' and is_active = 1 and deal_price >= '5' order by deal_type desc, title desc, deal_price desc ".$GLOBALS[sql_page]."";
								$sqlStrAux = "SELECT count(*) as total FROM ".TABLE_PRODUCT." where deal_coupon = 'd' and deal_sources_id = '".$_REQUEST['brand']."' and deal_price >= '5' and is_active = 1";
							}
							elseif($_REQUEST['category'] != '')
							{
								$sql = "select * from ".MANAGE_E_CATEGORY." where cat_id = '".$_REQUEST['category']."' and is_active = 1 and type='d' order by in_order asc";
								$res_subCat = mysql_query($sql);	
								?>
								<div>
								<div class="gradientbuttons">
									...
									<?php 
										...
										$subcatCount = 0;
										$newSubCatList = "";
										while($subcat = mysql_fetch_array($res_subCat))
										{
											if(isset($_REQUEST['subcategory']))
											{
												
										   	$subcatList = explode(',',$_REQUEST['subcategory']);
										   	
										   	$subcatCount = 0;
										   	foreach($subcatList as $subcatList)
										   	{
										   		
										   		if($subcatList==$subcat[0])
										   			$subcatCount++;	
										   	}
										   }	
										   ...
										   $sqlCount = "SELECT count(*) as total FROM ".TABLE_PRODUCT." where deal_coupon = 'd' and deal_price >= '5' and cat_id = '".$_REQUEST['category']."' and sub_cat_id ='".$subcat[0]."'  and is_active = 1";
											$result = mysql_query($sqlCount);
											$subCount = mysql_fetch_row($result);								
									?>
									
                           	<li>                          	
                           		<input type="checkbox" 
                           		onclick="window.location='<?php echo SITE_URL; ?>deal_store_subcategory.php?category=<?php echo $_REQUEST['category']; ?>&subcategory=<?php echo $subcat[0]; ?>'" name="<?php echo $subcat[1]; ?>" <?php if($subcat[0]==$_REQUEST['subcategory']) { ?> checked="checked" <?php } ?>/>
											                           		
                           		<a href="<?php echo SITE_URL; ?>deal_store_subcategory.php?category=<?php echo $_REQUEST['category']; ?>&subcategory=<?php echo $subcat[0]; ?>"><?php echo $subcat[2]; ?>
                           		<small><?php echo "(".$subCount[0].")"; ?></small>
                           		</a>
                           	</li>
                           <?php
                           		if($k%3==0)
                           ...
                           ?>
							...
							<?php 	
								...
								$sql_deal = "select * from ".TABLE_PRODUCT." where deal_coupon = 'd' and deal_price >= '5' and cat_id = '".$_REQUEST['category']."' and is_active = 1 order by deal_type desc, title desc, deal_price desc ".$GLOBALS[sql_page]."";
								$sqlStrAux = "SELECT count(*) as total FROM ".TABLE_PRODUCT." where deal_coupon = 'd' and deal_price >= '5' and cat_id = '".$_REQUEST['category']."' and is_active = 1";
								if(isset($_REQUEST['subcategory']) && $newSubCatList != '')
								{
									$sql_deal = "select * from ".TABLE_PRODUCT." where deal_coupon = 'd' and deal_price >= '5' and cat_id = '".$_REQUEST['category']."' and sub_cat_id in (".$newSubCatList.") and is_active = 1 order by deal_type desc, title desc, deal_price desc ".$GLOBALS[sql_page]."";
									$sqlStrAux = "SELECT count(*) as total FROM ".TABLE_PRODUCT." where deal_coupon = 'd' and deal_price >= '5' and cat_id = '".$_REQUEST['category']."' and sub_cat_id in (".$newSubCatList.")  and is_active = 1";
							...
							{
								$sql_deal = "select * from ".TABLE_PRODUCT." where deal_coupon = 'd' and is_active = 1 and deal_price >= '5' order by deal_type desc, title desc, deal_price desc ".$GLOBALS[sql_page]."";
