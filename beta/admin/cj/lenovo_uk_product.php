...
$arr = array("http://1805267:MAm+ZsAu@datatransfer.cj.com/datatransfer/files/1805267/outgoing/productcatalog/131365/Lenovo_UK-Product_Catalog.xml.gz");
...
function endElement($parser, $name){
	global $curr_tag,$curr_data,$program_name,$program_url,$catalog_name,$lastupdate,$product_name,$keyword,$product_description,$product_sku,$deal_currency,$deal_price,$deal_retailprice,$deal_buyurl,$deal_impressionurl,$deal_imageurl,$start_date1,$end_date1,$instock, $debugmode;
	...
	if($numcheck_dealsource == 0)
	{
        ...
        if (!$resinsert_dealsource && $debugmode===true) {
            $message  = '<b>Invalid query:</b><br>' . mysql_error() . '<br><br>'.'<b>Whole query:</b><br>' . $resinsert_dealsource . '<br><br>';
            echo $message;
        }else{
            //echo "<br />Deal Source Inserted Successfully<br />";
        }
	}
	...
	######################### PRODUCT INSERT STARTS ######################### 
		...
		if($numcheck_coupon == 0)
		{
			...
			if($catId>0){
                ...
                if (!$resinsert_coupon && $debugmode===true) {
                    $message  = '<b>Invalid query:</b><br>' . mysql_error() . '<br><br>'.'<b>Whole query:</b><br>' . $resinsert_dealsource . '<br><br>';
                    echo $message;
                }else{
                    //echo "<br />Product Inserted Successfully<br />";
                }
            }
		}
		else
		{
			...
			$sqlupdate_coupon = " UPDATE ".TABLE_PRODUCT." SET
								description='".mysql_real_escape_string($product_description)."',
			...
			if($catId>0){
                ...
                if (!$resupdate_coupon && $debugmode===true) {
                    $message  = '<b>Invalid query:</b><br>' . mysql_error() . '<br><br>'.'<b>Whole query:</b><br>' . $resinsert_dealsource . '<br><br>';
                    echo $message;
                }else{
                    //echo "<br />Product Updated Successfully<br />";
                }
            }
		}	
